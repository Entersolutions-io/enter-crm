...
export function LampEffect({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={cn(
        "relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-[#0A0A0B] w-full z-0",
        className
      )}
    >
      <div className="relative flex w-full flex-1 items-center justify-center isolate z-0">
        <motion.div
          ...
          className="absolute inset-auto right-1/2 h-56 overflow-visible w-[30rem] bg-gradient-conic from-indigo-500 via-transparent to-transparent [--conic-position:from_70deg_at_center_top]"
        >
        ...
        <div className="absolute top-1/2 h-48 w-full translate-y-12 bg-[#0A0A0B] blur-2xl" />
        ...
        <div className="absolute inset-auto z-40 h-44 w-full -translate-y-[12.5rem] bg-[#0A0A0B]" />
      </div>
